...
%forest, and contains the index r for each mser in the
%tree.
...
maskAll = zeros(size(img,1), size(img,2),'uint32');
newSector = 1;
...
%     sel = find(auxMask == 1);
    ...
    sector = mask(sel(1)); %Provided that the MSERs are find, this should be as good as count_unique
    ...
    if sector == 0 %This is a new sector!
        ...
        if positions(sector)>2
            i = maskAll(sel(1));%i = count_unique(maskAll(sel));
...
for k = 1:numel(forest)
    ...
    %         forest{1} = [1 2 1;1 3 1;1 4 1;1 5 1;1 6 1;2 7 2;2 8 2;3 9 2;3 10 3;4 11 2;4 12 2;...
    %             5 13 2;5 14 2;6 15 2;6 16 2];
    %         references{1} = [1 0.2;2 0.1;3 0.5; 4 0.1;5 0.6;6 0.8;7 0.4;8 0.5;9 0.5;10 0.5;...
    %             11 0.35;12 0.6;13 0.1;14 0.2; 15 0.2;16 0.6];
    %         tree = forest{1};
    %         k = 1;largeVal = -50;
